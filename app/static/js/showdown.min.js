var Showdown={extensions:{}};var forEach=Showdown.forEach=function(obj,callback){if(typeof obj.forEach==="function")obj.forEach(callback);else{var i,len=obj.length;for(i=0;i<len;i++)callback(obj[i],i,obj)}};var stdExtName=function(s){return s.replace(/[_-]||\s/g,"").toLowerCase()};
...
Showdown.extensions[name]=require("./extensions/"+ext)})}}this.makeHtml=function(text){g_urls={};g_titles={};g_html_blocks=[];text=text.replace(/~/g,"~T");text=text.replace(/\$/g,"~D");text=text.replace(/\r\n/g,"\n");text=text.replace(/\r/g,"\n");text="\n\n"+text+"\n\n";text=_Detab(text);text=text.replace(/^[ \t]+$/mg,"");Showdown.forEach(g_lang_extensions,function(x){text=_ExecuteExtension(x,text)});text=_DoGithubCodeBlocks(text);text=_HashHTMLBlocks(text);text=_StripLinkDefinitions(text);text=_RunBlockGamut(text);
...
"&amp;");text=text.replace(/<(?![a-z\/?\$!])/gi,"&lt;");return text};var _EncodeBackslashEscapes=function(text){text=text.replace(/\\(\\)/g,escapeCharacters_callback);text=text.replace(/\\([`*_{}\[\]()>#+-.!])/g,escapeCharacters_callback);return text};var _DoAutoLinks=function(text){text=text.replace(/<((https?|ftp|dict):[^'">\s]+)>/gi,'<a href="$1">$1</a>');text=text.replace(/<(?:mailto:)?([-.\w]+\@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)>/gi,function(wholeMatch,m1){return _EncodeEmailAddress(_UnescapeSpecialChars(m1))});
